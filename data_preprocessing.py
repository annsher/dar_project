...
def create_table_intern(conn_i):
    cursor = conn_i.cursor()
    f = open('create_tables_for_DDS.txt', 'r')
    create = ''.join(f.readlines())
    cursor.execute(create)
    truncate_table_intern('pos', conn_i)
    #загрузка таблицы магазинов, которую предоставил заказчик
    csv_to_db('pos', 'pos, pos_name', conn_i)
    conn_i.commit()
    cursor.close()
...
def record_errors(records, table, error_name, conn):
    ...
    if len(error) != 0:
        ...
        csv_to_db('error', ','.join(list(error)), conn)
        print(f'error table updated: {error_name} {table}')



def brand_table_processing(conn_z, conn_i):
    truncate_table_intern('brand', conn_i)
    cursor = conn_z.cursor()
    select = 'SELECT * FROM sources.brand'
    cursor.execute(select)
    records = cursor.fetchall()
    brand = pd.DataFrame(list(records), columns = ['brand_id', 'brand'])
    ...
    record_errors(brand[brand.duplicated(subset=['brand_id'])], 'brand', 'дубликат id', conn_i)
    brand = brand.drop_duplicates(subset=['brand_id'])
    record_errors(brand[(pd.to_numeric(brand['brand_id'], errors='coerce') < 0) | (str(pd.to_numeric(brand['brand_id'], errors='coerce')) == 'nan')], 'brand','неверный формат id', conn_i)
    ...
    csv_to_db('brand', ','.join(list(brand)), conn_i)
    print('brand table uploaded')

def category_table_processing(conn_z, conn_i):
    truncate_table_intern('category', conn_i)
    cursor = conn_z.cursor()
    select = 'SELECT * FROM sources.category'
    cursor.execute(select)
    records = cursor.fetchall()
    category = pd.DataFrame(list(records), columns=['category_id', 'category_name'])
    category = category.drop_duplicates()
    record_errors(category[category.duplicated(subset=['category_id'])], 'category', 'дубликат id', conn_i)
    category = category.drop_duplicates(subset=['category_id'])
    record_errors(category[(category['category_id'] == '') | (category['category_id'] == 'NULL')| (category['category_id'] == 'nan')], 'category', 'неверный формат id', conn_i)
    ...
    csv_to_db('category', ','.join(list(category)), conn_i)
    print('category table uploaded')

def product_table_processing(conn_z, conn_i):
    truncate_table_intern('product', conn_i)
    cursor = conn_z.cursor()
    select = 'SELECT * FROM sources.product'
    cursor.execute(select)
    records = cursor.fetchall()
    product = pd.DataFrame(list(records), columns=['product_id', 'name_short', 'category_id', 'pricing_line_id', 'brand_id'])
    select = 'SELECT * FROM sources.category'
    cursor.execute(select)
    records = cursor.fetchall()
    category = pd.DataFrame(list(records), columns=['category_id', 'category'])
    merged = product.merge(category, how='left', on='category_id')
    merged['category'] = merged['category'].astype('str')
    error = merged[merged['category'] == 'nan']
    records = []
    for i in range(len(error)):
    ...
    error = pd.DataFrame(list(records), columns=['data', 'error', 'table_name'])
    error.to_csv('error.csv', index=False)
    csv_to_db('error', ','.join(list(error)), conn_i)
    print('error table updated: категории не существует')
    product = product.drop_duplicates()
    record_errors(product[product.duplicated(subset=['product_id', 'brand_id'])], 'product', 'дубликат product_id, brand_id', conn_i)
    product = product.drop_duplicates(subset=['product_id', 'brand_id'])
    record_errors(product[product.duplicated(subset=['name_short', 'brand_id'])], 'product', 'дубликат name_short, brand_id', conn_i)
    ...
    record_errors(product[product.duplicated(subset=['product_id'])], 'product','дубликат id', conn_i)
    product = product.drop_duplicates(subset=['product_id'])
    record_errors(product[(pd.to_numeric(product['product_id'], errors='coerce') < 0) | (
                str(pd.to_numeric(product['product_id'], errors='coerce')) == 'nan')], 'product', 'неверный формат product_id', conn_i)
    product = product[(product['category_id'] == '') | (product['category_id'] == 'NULL') | (product['category_id'] != 'nan')]
    record_errors(product[(pd.to_numeric(product['category_id'], errors='coerce') < 0) | (str(pd.to_numeric(product['category_id'], errors='coerce')) == 'nan')], 'product', 'неверный формат category_id', conn_i)
    product = product[(product['category_id'] != '') & (product['category_id'] != 'NULL') & (product['category_id'] != 'nan')]
    record_errors(product[(pd.to_numeric(product['pricing_line_id'], errors='coerce') < 0) | (str(pd.to_numeric(product['pricing_line_id'], errors='coerce')) == 'nan')], 'product', 'неверный формат pricing_line_id', conn_i)
    product = product[pd.to_numeric(product['pricing_line_id'], errors='coerce') >= 0]
    record_errors(product[(pd.to_numeric(product['brand_id'], errors='coerce') < 0) | (str(pd.to_numeric(product['brand_id'], errors='coerce')) == 'nan')], 'product', 'неверный формат brand_id', conn_i)
...
def stock_table_processing(conn_z, conn_i):
    truncate_table_intern('stock', conn_i)
    cursor = conn_i.cursor()
    select = 'SELECT * FROM "DDS".product'
    cursor.execute(select)
    records = cursor.fetchall()
    product = pd.DataFrame(list(records), columns=['product_id', 'name_short', 'category_id', 'pricing_line_id', 'brand_id'])
    conn_i.commit()
    cursor.close()
    cursor = conn_z.cursor()
    select = 'SELECT * FROM sources.stock'
    cursor.execute(select)
    records = cursor.fetchall()
    stock = pd.DataFrame(list(records), columns=['available_on', 'product_id', 'pos', 'available_quantity', 'cost_per_item'])
    conn_z.commit()
    cursor.close()
    stock = stock.drop_duplicates()
    record_errors(stock[(pd.to_numeric(stock['available_on'], errors='coerce') < 0) & (~(fnmatch(str(stock['available_on']), '????-??-??*')))], 'stock', 'неверный формат available_on', conn_i)
    ...
    record_errors(product[product.duplicated(subset=['product_id', 'brand_id'])], 'stock', 'неверный формат product_id', conn_i)
    ...
    record_errors(stock[(pd.to_numeric(stock['cost_per_item'], errors='coerce') <= 0) | (str(pd.to_numeric(stock['cost_per_item'], errors='coerce')) == 'nan')], 'stock',
                  'неверный формат cost_per_item', conn_i)
    ...
    cursor = conn_i.cursor()
    select = 'SELECT * FROM "DDS".pos'
    cursor.execute(select)
    records = cursor.fetchall()
    pos = pd.DataFrame(list(records), columns=['pos', 'pos_name'])
    conn_i.commit()
    cursor.close()
...
def transaction_table_processing(conn_z, conn_i):
    ...
    product = pd.DataFrame(list(records), columns=['product_id', 'name_short', 'category_id', 'pricing_line_id', 'brand_id'])
    cursor = conn_z.cursor()
    select = 'SELECT * FROM sources.transaction'
    cursor.execute(select)
    records = cursor.fetchall()
    transaction = pd.DataFrame(list(records), columns=['transaction_id', 'product_id', 'recorded_on', 'quantity', 'price', 'price_full', 'order_type_id'])
    transaction = transaction.drop_duplicates()
    record_errors(transaction[(transaction['transaction_id'] == '') | (transaction['transaction_id'] == 'NULL')], 'transaction', 'пустое значение transaction_id', conn_i)
    ...
    record_errors(transaction[(pd.to_numeric(transaction['product_id'], errors='coerce') < 0) | (str(
        pd.to_numeric(transaction['product_id'], errors='coerce')) == 'nan')], 'transaction', 'неверный формат product_id', conn_i)
    ...
    record_errors(transaction[pd.to_numeric(transaction['quantity'], errors='coerce') < 0], 'transaction', 'неверный формат quantity', conn_i)
    ...
    record_errors(transaction[pd.to_numeric(transaction['price_full'], errors='coerce') <= 0], 'transaction',
                  'неверный формат price_full', conn_i)
    ...
    csv_to_db('transaction', ','.join(list(transaction)), conn_i)
