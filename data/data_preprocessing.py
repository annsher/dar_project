...
def csv_to_db(table, columns, conn):
    cursor = conn.cursor()
    f = open(f"/opt/airflow/data/{table}.csv", 'r', encoding="utf-8")
...
def create_table_intern(conn_i):
    ...
    f = open('/opt/airflow/data/create_tables_for_DDS.sql', 'r')
...
def transaction_table_processing(conn_z, conn_i):
    ...
    merged = merged[merged['name_short'] != 'nan']
    merged = merged[merged['name_short'] != 'None']
    error = pd.DataFrame(records, columns=['data', 'error', 'table_name'])
    error.to_sql('error', conn_i.connect(), 'DDS', 'append', index = False, method='multi')
...
conn_i = create_engine("postgresql+psycopg2://interns_4:dlf6y?@10.1.108.29:5432/internship_4_db",pool_size=10, max_overflow=30)
conn_z = create_engine("postgresql+psycopg2://interns_4:dlf6y?@10.1.108.29:5432/internship_sources",pool_size=10, max_overflow=30)
